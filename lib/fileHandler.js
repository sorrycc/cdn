...
var FileHandler = {
    ...
    each: function(fn, callback) {
        var that = this;
        async.forEach(Object.keys(this.Files).sort(), function(item, done) {
    ...
    upload: function(callback) {
        ...
        console.info('Uploading by ' + this.Config.username + '...');
        this.each(function(file, done) {
    ...
    postUpload: function(onComplete) {
        var that = this;
        var copyUrls = [];
        ...
        this.each(function(file, done) {
            ...
            onComplete && onComplete(copyUrls.length > 1 ? copyUrls[0] : copyUrls);            
            // copy to clipborad in MacOS
            exec('echo "' + copyUrls.join('\\n') + '\\c" | pbcopy', {silent: true});
        });
