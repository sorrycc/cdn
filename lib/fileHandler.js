...
var FileHandler = {
    ...
    upload: function(callback) {
        ...
        process.stdout.write('Uploading by ' + this.Config.username.grey + ' ');
        var inter = setInterval(function() {
            process.stdout.write(".");
        }, 1000);

        this.each(function(file, done) {
            file.data && needle.post(that.Config.url, file.data, { multipart: true, timeout: 0 }, function(error, response, body) {
                clearInterval(inter);
                console.log();
    ...
    _postOneFileUpload: function(file, body, done) {
        ...
        CheckUrl(hostUrl, function() {
            ...
            that.error('Maybe something wrong, or check your web.');
            done();
        });
    },
